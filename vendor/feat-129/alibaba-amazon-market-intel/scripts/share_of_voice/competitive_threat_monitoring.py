"""Competitive Threat Monitoring — Track competitor SOV changes, detect ad offensive escalation and new entrants.
...
def analyze(mcp_data, output_dir=None, keyword=''):
    ...
    if not df.empty:
        mean_sponsored = df['sponsored_sov_weighted'].mean()
        df['threat_flag'] = df['sponsored_sov_weighted'].apply(
            lambda x: 'high_ad_spend' if x and x > mean_sponsored * 1.5 else 'normal')

    csv_path = os.path.join(data_dir, 'competitive_threat_monitoring.csv')
