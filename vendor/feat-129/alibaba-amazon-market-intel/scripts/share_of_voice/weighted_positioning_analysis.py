"""Weighted Positioning Analysis — Compare basic vs weighted SOV, assess page position quality and Amazon's Choice badge impact.
...
def analyze(mcp_data, output_dir=None, keyword=''):
    ...
    for item in (items or []):
        ...
        for brand_item in brands:
            basic = brand_item.get('combined_basic_sov', 0) or 0
            weighted = brand_item.get('combined_weighted_sov', 0) or 0
            all_rows.append({
                'keyword': keyword,
                ...
                'combined_sov_basic': basic,
                'combined_sov_weighted': weighted,
                'position_quality_gap': weighted - basic,
                'combined_avg_position': brand_item.get('combined_average_position'),
    ...
    if not df.empty:
        df['position_quality'] = df['position_quality_gap'].apply(
            lambda x: 'premium' if x and x > 0.05 else 'poor' if x and x < -0.05 else 'neutral')

    csv_path = os.path.join(data_dir, 'weighted_positioning_analysis.csv')
