"""Conversion Performance — Analyze Top 3 ASIN click and conversion data per keyword.
...
def analyze(mcp_data, output_dir=None, keyword=''):
    ...
    for item in (items or []):
        ...
        top_asins = attr_dict.get('top_asins', []) or []
        for rank, asin_data in enumerate(top_asins[:3], 1):
            all_rows.append({
                'keyword': keyword,
                'asin': asin_data.get('asin'),
                'name': asin_data.get('name'),
                'brand': asin_data.get('brand'),
                'click_rank': rank,
                'clicks': asin_data.get('clicks'),
                'conversions': asin_data.get('conversions'),
                'conversion_rate': asin_data.get('conversion_rate'),
    ...
    csv_path = os.path.join(data_dir, 'conversion_performance.csv')
