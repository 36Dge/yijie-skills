"""New Product Launch Monitoring — Filter by listing date for products launched in the last 30/60/90 days, track BSR trajectory and review velocity."""
...
def analyze(mcp_data, output_dir=None, days_back=90, date_threshold=''):
    ...
    for item in (items or []):
        ...
        date_available = attrs.get('date_first_available')
        if not (date_available and str(date_available) >= date_threshold):
            continue
        rows.append({
    ...
    csv_path = os.path.join(data_dir, 'new_launches.csv')
