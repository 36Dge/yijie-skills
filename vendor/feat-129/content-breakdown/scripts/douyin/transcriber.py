...
def transcribe_whisper(audio_path: Path, model_name: str = "tiny") -> dict | None:
...
def process_video(video_dir: Path) -> dict:
    """完整处理单个视频：提取音频 → 直接 Whisper 转录（跳过人声分离）"""
    ...
    # 优化：使用 tiny 模型，速度提升 3-4 倍（精度略有下降）
    transcript = transcribe_whisper(audio_path, "tiny")
    if transcript:
        result["success"] = True
        ...
        print(f"  ✓ 转录完成（tiny 模型），字数：{len(result['transcript'])}")
    else:
